...
mod tests {
    ...
    #[test]
    fn proxy_response_accepts_pass_rule_and_future_types() -> Result<(), serde_json::Error> {
        let response: Proxies = serde_json::from_str(
            r#"{
                "proxies": {
                    "PASS-RULE": { "name": "PASS-RULE", "type": "PassRule" },
                    "future": { "name": "future", "type": "FutureProxyType" }
                }
            }"#,
        )?;

        assert!(matches!(&response.proxies["PASS-RULE"].proxy_type, ProxyType::PassRule));
        assert!(matches!(
            &response.proxies["future"].proxy_type,
            ProxyType::Unknown(value) if value == "FutureProxyType"
        ));
